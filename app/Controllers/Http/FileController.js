...
class FileController {
  ...
  async store ({ request, response }) {
    try{
      ...
      await upload.move(Helpers.appRoot('img/pefil'),{
        name: fileName
      })
  ...
  async show({params,response}){
    ...
    return response.download(Helpers.appRoot(`img/perfil/${file.file}`))
  ...
  async destroy ({ params, response }) {
    ...
    try {
      ...
      await file.delete()
      fs.unlinkSync(Helpers.appRoot(`img/perfil/${file.file}`))
